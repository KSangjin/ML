...
data = [[float(i[0])/10, float(i[1]), float(i[2])/100] for i in df.values]
labels = [float(i[3].replace('\xa0', ''))/1000 for i in df.values]

testdata = pd.read_excel('테트리스 test case.xlsx') 
testdf = pd.DataFrame(testdata, columns=['APM', 'PPS', 'VS', 'TR'])
testdata = [[float(i[0])/10, float(i[1]), float(i[2])/100] for i in testdf.values]
...
for _ in range(100):
    ...
    for i in range(len(FinalzValue)):
        loss += abs(FinalzValue[i]-testlabels[i])/testlabels[i]
